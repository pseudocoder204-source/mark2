...
- **`triage`** (pure Python, `run_triage` → `_fallback_order`): orders findings by severity tier then CVSS, descending — no LLM call. `notes/FinetuneGuideTriage.txt` Phase 5 (2026-07-12) tuned three models (3B multi-turn, 3B single-shot, 7B multi-turn) against this deterministic ordering and none beat it on held-out intra-tier Kendall-tau, so no triage LLM is shipped or invoked; the artifacts are kept for reference only, in case a future corpus/base clears the bar.
