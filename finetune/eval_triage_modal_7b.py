"""FinetuneGuideTriage.txt PHASE 5 escalation — fork of eval_triage_modal.py
pointed at the 7B multi-turn adapter (mark2-triage-lora-7b-out), after both
3B variants (multi-turn and single-shot) failed the Step 14 A/B floor.
Generate triage outputs from BOTH the tuned 7B LoRA adapter and the stock
(untuned) 7B base model over held-out finetune/eval_triage.jsonl, with REAL
...
app = modal.App("mark2-triage-lora-7b-eval")

volume = modal.Volume.from_name("mark2-triage-lora-7b-out", create_if_missing=False)
...
ADAPTER_DIR = "/output/triage-7b-lora"
MAX_NEW_TOKENS = 1024
...
@app.cls(image=image, gpu="A100-40GB", volumes={"/output": volume}, timeout=1800, scaledown_window=600)
class TriageModel:
...
def main():
    ...
    out_path = _REPO_ROOT / "finetune" / "eval_triage_7b_outputs.json"
